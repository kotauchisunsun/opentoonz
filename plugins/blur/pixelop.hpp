...
void hv_op(T *__restrict ptr, const T *__restrict src, int w, int h,
...
template <typename T, int component>
void hv_op(const T *__restrict src, int w, int h, int stride,
           std::function<void(T &&, T &&, T &&, int, int)> &&f) {
  for (int i = 0; i < h; i++) {
    for (int j = 0; j < w; j++) {
      T c0 = src[j * component];
      T c1 = src[j * component + 1];
...
template <typename T>
void hv_op(T *__restrict ptr, const T *__restrict src, int w, int h, int stride,
...
template <typename T>
void hv_kernel(T *__restrict ptr, const T *__restrict src, int w, int h,
               int stride, std::function<T(const T *, int, int, int)> &&f) {
  for (int i = 0; i < h; i++) {
    for (int j = 0; j < w; j++) {
...
void hv_kernel(
    T *__restrict ptr, const T *__restrict src, int w, int h, int dstride,
    int sstride,
    std::function<void(T[components], const T *, int, int, int)> &&f) {
