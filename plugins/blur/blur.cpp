...
template <>
void dump_value<toonz_param_traits_double_t::iovaluetype>(
    const toonz_param_traits_double_t::iovaluetype &v) {
  printf("value: type:%s {%g}\n",
         typeid(toonz_param_traits_double_t::iovaluetype).name(), v);
}

template <>
void dump_value<toonz_param_traits_range_t::iovaluetype>(
    const toonz_param_traits_range_t::iovaluetype &v) {
  printf("value: type:%s {%g, %g}\n",
         typeid(toonz_param_traits_range_t::iovaluetype).name(), v.a, v.b);
}

template <>
void dump_value<toonz_param_traits_point_t::iovaluetype>(
    const toonz_param_traits_point_t::iovaluetype &v) {
  printf("value: type:%s {%g, %g}\n",
         typeid(toonz_param_traits_point_t::iovaluetype).name(), v.x, v.y);
}

template <>
void dump_value<toonz_param_traits_color_t::iovaluetype>(
    const toonz_param_traits_color_t::iovaluetype &v) {
  printf("value: type:%s {%d, %d, %d, %d}\n",
         typeid(toonz_param_traits_color_t::iovaluetype).name(), v.c0, v.c1,
         v.c2, v.m);
}

template <>
void dump_value<toonz_param_traits_spectrum_t::iovaluetype>(
    const toonz_param_traits_spectrum_t::iovaluetype &v) {
  printf("value: type:%s {%g, %g, %g, %g, %g}\n",
         typeid(toonz_param_traits_spectrum_t::iovaluetype).name(), v.w, v.c0,
         v.c1, v.c2, v.m);
}

template <>
void dump_value<toonz_param_traits_tonecurve_t::iovaluetype>(
    const toonz_param_traits_tonecurve_t::iovaluetype &v) {
  printf("value: type:%s {%g, %g, %d, %d}\n",
         typeid(toonz_param_traits_tonecurve_t::iovaluetype).name(), v.x, v.y,
         v.channel, v.interp);
}
...
template <>
bool get_and_check<toonz_param_traits_string_t>(toonz::node_handle_t node,
...
template <>
bool get_and_check<toonz_param_traits_spectrum_t>(toonz::node_handle_t node,
                                                  const char *nm, double frame,
                                                  double opt) {
  auto nodeif = grab_interf<toonz::node_interface_t>(TOONZ_UUID_NODE);
  auto pif    = grab_interf<toonz::param_interface_t>(TOONZ_UUID_PARAM);
...
template <>
bool get_and_check<toonz_param_traits_tonecurve_t>(toonz::node_handle_t node,
                                                   const char *nm, double frame,
                                                   double opt) {
  auto nodeif = grab_interf<toonz::node_interface_t>(TOONZ_UUID_NODE);
  auto pif    = grab_interf<toonz::param_interface_t>(TOONZ_UUID_PARAM);
...
void do_compute(toonz::node_handle_t node, const toonz::rendering_setting_t *rs,
  ...
  toonz::port_handle_t port = nullptr;
  {
    ...
    if (nodeif) {
      ...
      if (pif) {
        ...
        int ret =
            pif->get_string_value(str, &sz, l / 2 + 1, shortage_buf.data());
  ...
  if (port && tileif && portif) {
    ...
    if (fxif) {
      ...
      hv_kernel<uint8_t, 4>(
          daddr, saddr + sstride + 4, lx, ly, dstride, sstride,
          [](uint8_t v[4], const uint8_t *p, int stride, int xinbytes,
             int yinbytes) {
            ...
            /* TODO:
                            toonz の TPixel は char r, g, b, m;
               のように持っているのでそのまま uin32_t だとエンディアンが変わる.
                            toonz ビルド時の configuration
...
param_desc_t params0_[] = {
    param_desc_ctor<traits_double_t>(
        "first", "1st", {1.0 /* default */, -1.0 /* min */, 1.0 /* max */},
        "description of the first param"),
    param_desc_ctor<traits_range_t>(
        "second", "2nd",
        {{48.0, 64.0} /* default */, {0.0, 180.0} /* min-max */}),
    param_desc_ctor<traits_color_t>("third", "3rd",
                                    {0xff, 0xff, 0xff, 0xff} /* white */),
    param_desc_ctor<traits_point_t>("fourth", "4th",
                                    {{64, -64} /* default */,
                                     {-200, 200} /* min */,
                                     {-200, +200} /* max */}),
};
...
param_desc_t params1_[] = {
    ...
    param_desc_ctor<traits_int_t>("sixth", "6th", {2, -10, 10},
                                  "this is integer"),
    param_desc_ctor<traits_bool_t>("seventh", "7th", {1}),
};

toonz_param_traits_spectrum_t::valuetype points_[] = {
    {0.0, 1.0, 0.0, 0.0, 1.0},
    {0.5, 0.5, 0.0, 0.5, 1.0},
    {1.0, 0.0, 0.0, 0.5, 1.0}};

param_desc_t params2_[] = {
    param_desc_ctor<traits_string_t>("eighth", "8th",
    ...
    param_desc_ctor<traits_tonecurve_t>("tenth", "10th",
                                        {/* tonecurve has no default */}),
};

toonz_param_group_t groups0_[] = {
    param_group_ctor("Group1", sizeof(params0_) / sizeof(param_desc_t),
                     params0_),
    param_group_ctor("Group2", sizeof(params1_) / sizeof(param_desc_t),
                     params1_),
    param_group_ctor("Group3", sizeof(params2_) / sizeof(param_desc_t),
                     params2_),
};

toonz_param_page_t pages_[] = {
    param_page_ctor("Properties",
...
TOONZ_PLUGIN_PROBE_DEFINE(TOONZ_PLUGIN_VER(1, 0), "blur-plugin" /* name */,
                          "dwango" /* vendor */,
                          "libblur.plugin" /* identifier */,
                          "a blur plugin for test", "http://dwango.co.jp/",
