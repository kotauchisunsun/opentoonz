...
public:
  ...
  ToonzAffine(double a11, double a12, double a13, double a21, double a22,
              double a23)
...
inline ToonzPoint ToonzAffine::operator*(const ToonzPoint &pt) const {
  return ToonzPoint(pt.x * a11 + pt.y * a12 + a13,
                    pt.x * a21 + pt.y * a22 + a23);
}
...
inline ToonzRect ToonzAffine::operator*(const ToonzRect &r) const {
  if (r.x0 == -std::numeric_limits<double>::max() ||
    ...
    return ToonzRect(-std::numeric_limits<double>::max(),
  ...
  return ToonzRect(std::min(std::min(p0.x, p1.x), std::min(p2.x, p3.x)),
...
ToonzAffine ToonzAffine::operator*(const ToonzAffine &toonzAffine) const {
  return ToonzAffine(a11 * toonzAffine.a11 + a12 * toonzAffine.a21,
...
ToonzAffine ToonzAffine::inv() const {
  if (equals(a12, 0.0) && equals(a21, 0.0)) {
    ...
    return ToonzAffine(inv_a11, 0.0, -a13 * inv_a11, 0.0, inv_a22,
                       -a23 * inv_a22);
    ...
    return ToonzAffine(0.0, inv_a21, -a23 * inv_a21, inv_a12, 0.0,
                       -a13 * inv_a12);
  }
  double inv_det = 1.0 / det();
  return ToonzAffine(a22 * inv_det, -a12 * inv_det,
                     (a12 * a23 - a22 * a13) * inv_det, -a21 * inv_det,
                     a11 * inv_det, (a21 * a13 - a11 * a23) * inv_det);
}

double ToonzAffine::det() const { return a11 * a22 - a12 * a21; }

bool ToonzAffine::isIdentity(double err) const {
  double value = (a11 - 1.0) * (a11 - 1.0) + (a22 - 1.0) * (a22 - 1.0) +
                 a12 * a12 + a13 * a13 + a21 * a21 + a23 * a23;
  return value < err;
}
...
ToonzAffine ToonzAffine::place(double u, double v, double x, double y) const {
  return ToonzAffine(a11, a12, x - (a11 * u + a12 * v), a21, a22,
                     y - (a21 * u + a22 * v));
}
