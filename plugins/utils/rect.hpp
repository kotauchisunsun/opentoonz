...
ToonzRect ToonzRect::operator*(const ToonzRect &toonzRect) const {
  if (isEmpty() || toonzRect.isEmpty() || toonzRect.x1 < x0 ||
      x1 < toonzRect.x0 || toonzRect.y1 < y0 || y1 < toonzRect.y0) {
    return ToonzRect();
  }
...
bool ToonzRect::operator==(const ToonzRect &toonzRect) const {
  return equals(x0, toonzRect.x0) && equals(y0, toonzRect.y0) &&
         equals(x1, toonzRect.x1) && equals(y1, toonzRect.y1);
}

bool ToonzRect::isEmpty() const {
  if ((equals(x0, x1) && equals(y0, y1)) || x0 > x1 || y0 > y1) {
    return true;
  }
  return false;
}
...
bool ToonzRect::isContained(const ToonzRect &toonzRect) const {
  return toonzRect.x0 <= x0 && x1 <= toonzRect.x1 && toonzRect.y0 <= y0 &&
         y1 <= toonzRect.y1;
}

bool ToonzRect::isOverlapped(const ToonzRect &toonzRect) const {
  return x0 <= toonzRect.x1 && x1 >= toonzRect.x0 && y0 <= toonzRect.y1 &&
         y1 >= toonzRect.y0;
}
