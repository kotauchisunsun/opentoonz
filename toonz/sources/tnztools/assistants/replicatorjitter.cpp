...
public:
  TReplicatorJitter(TMetaObject &object)
    ...
    addProperty(
        new TBoolProperty(m_idKeepFirstPoint.str(), getKeepFirstPoint()));
    addProperty(new TBoolProperty(m_idKeepLastPoint.str(), getKeepLastPoint()));
  }

  static QString getLocalName() { return tr("Jitter"); }
  ...
  inline double getPeriod() const { return data()[m_idPeriod].getDouble(); }
...
protected:
  ...
  double getScale(const TAffine &a) const {
    return sqrt(a.a11 * a.a11 + a.a12 * a.a12 + a.a21 * a.a21 + a.a22 * a.a22) /
           2;
  }
...
public:
  void getPoints(const TAffine &toTool, PointList &points) const override {
    ...
    if (period > TConsts::epsilon && amplitude > TConsts::epsilon) {
      ...
      for (int i = i0; i < i1; ++i) {
        points[i].x +=
            TModifierJitter::func(seedX, points[i].x / period) * amplitude;
        points[i].y +=
            TModifierJitter::func(seedY, points[i].y / period) * amplitude;
        ++seedX, ++seedY;
      }
    }
  }
  ...
  void getModifiers(const TAffine &toTool,
    ...
    outModifiers.push_back(new TModifierJitter(
        getPeriod() * scale, getAmplitude() * scale, getSkipFirst(),
        getSkipLast(), getKeepFirstPoint(), getKeepLastPoint()));
  }
  ...
  void draw(TToolViewer *, bool enabled) const override {
    ...
    drawSegment(TPointD(c.x - h, c.y), TPointD(c.x - q, c.y + a), pixelSize,
                alpha);
    drawSegment(TPointD(c.x - q, c.y + a), TPointD(c.x + q, c.y - a), pixelSize,
                alpha);
    drawSegment(TPointD(c.x + q, c.y - a), TPointD(c.x + h, c.y), pixelSize,
                alpha);
  }
};
