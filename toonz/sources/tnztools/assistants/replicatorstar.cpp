...
public:
  ...
  static QString getLocalName() { return tr("Replicator Star"); }
  ...
  inline bool getMirror() const { return data()[m_idMirror].getBool(); }
...
protected:
  ...
  void onMovePoint(TAssistantPoint &point, const TPointD &position) override {
    if (&point == &m_center) m_a.position += position - m_center.position;
...
public:
  ...
  void getPoints(const TAffine &toTool, PointList &points) const override {
    ...
    TAffine t2(x.x, -y.x, c.x, x.y, -y.y, c.y);

    TAffine t0 = t1.inv();
    TRotation r(360.0 / getCount());

    for (int i = 0; i < count; ++i) {
      if (i) transformPoints(t1 * t0, points, i0, i1);
      if (mirror) {
  ...
  void getModifiers(const TAffine &toTool,
    ...
    for (int i = 0; i < count; ++i) {
      if (i) modifier->transforms.push_back(TTrackTransform(t1 * t0));
      if (mirror) {
  ...
  void draw(TToolViewer *, bool enabled) const override {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      if (TGuidelineLine::truncateRay(oneBox, pp0, pp1))
        drawSegment(matrixInv * pp0, matrixInv * pp1, pixelSize,
                    i ? gridAlpha : alpha);
      p0 = r * (p0 - c) + c;
      p1 = r * (p1 - c) + c;
    }
