...
void TAssistantVanishingPoint::fixCenter() {
  if (!(m_a0.position == m_a1.position) && !(m_b0.position == m_b1.position)) {
...
void TAssistantVanishingPoint::fixSidePoint(TAssistantPoint &p0,
                                            TAssistantPoint &p1,
                                            TPointD previousP0) {
...
void TAssistantVanishingPoint::fixGrid1(const TPointD &previousCenter,
                                        const TPointD &previousGrid0) {
...
void TAssistantVanishingPoint::onMovePoint(TAssistantPoint &point,
...
void TAssistantVanishingPoint::getGuidelines(
    const TPointD &position, const TAffine &toTool, const TPixelD &color,
  ...
  if (getPassThrough()) {
    outGuidelines.push_back(TGuidelineP(
        new TGuidelineInfiniteLine(getEnabled(), getMagnetism(), color,
                                   toTool * m_center.position, position)));
  } else {
    outGuidelines.push_back(
        TGuidelineP(new TGuidelineRay(getEnabled(), getMagnetism(), color,
                                      toTool * m_center.position, position)));
  }
}

void TAssistantVanishingPoint::drawSimpleGrid(const TPointD &center,
                                              const TPointD &grid0,
                                              const TPointD &grid1,
                                              double alpha) {
...
void TAssistantVanishingPoint::drawPerspectiveGrid(const TPointD &center,
                                                   const TPointD &grid0,
                                                   const TPointD &grid1,
                                                   double alpha) {
  ...
  double dk    = 1 / k;
  TAffine gridMatrix(ox.x, oy.x, center.x, ox.y, oy.y, center.y);
