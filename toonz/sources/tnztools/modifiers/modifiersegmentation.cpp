...
void TModifierSegmentation::setMaxLevel(int maxLevel) {
...
void TModifierSegmentation::addSegments(TTrack &track, const TTrackPoint &p0,
  ...
  --level;
  TTrackPoint p =
      track.calcPointFromOriginal(0.5 * (p0.originalIndex + p1.originalIndex));
  addSegments(track, p0, p, level);
  addSegments(track, p, p1, level);
}

void TModifierSegmentation::modifyTrack(const TTrack &track,
