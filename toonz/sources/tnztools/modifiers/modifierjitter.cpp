...
public:
  ...
  static TPointD getPoint(unsigned int seed, int i, double *prevX = nullptr) {
    ...
    if (dx < 0.5) {
      ...
      px += 0.5 - i;
      if (dx < px) dx = randomNext(pseed) * (1 - px) + px;
    }
  ...
  static inline double spline(double x, TPointD *points) {
    ...
    return y0 * (2 * lll - 3 * ll + 1) + y1 * (-2 * lll + 3 * ll) +
           t0 * (lll - 2 * ll + l) + t1 * (lll - ll);
  }
...
TModifierJitter::Interpolator::Interpolator(TTrack &track, double period,
                                            double amplitude,
                                            bool keepFirstPoint,
                                            bool keepLastPoint)
...
TTrackPoint TModifierJitter::Interpolator::interpolateFromOriginal(
  ...
  if (frequency && a && track.original && (keepFirstPoint || keepLastPoint)) {
    ...
    if (ll < TConsts::epsilon) {
      a = 0;
    } else if (keepFirstPoint && keepLastPoint && ll < 2) {
      a *= 0.5 - 0.5 * cos(l / ll * M_2PI);
    } else if (keepFirstPoint && l < 1) {
      a *= 0.5 - 0.5 * cos(l * M_PI);
    } else if (keepLastPoint && (ll - l) < 1) {
...
TModifierJitter::TModifierJitter(double period, double amplitude, int skipFirst,
                                 int skipLast, bool keepFirstPoint,
                                 bool keepLastPoint)
    : period(period)
    , amplitude(amplitude)
    , skipFirst(skipFirst)
...
void TModifierJitter::modifyTrack(const TTrack &track, TTrackList &outTracks) {
  if (!track.handler && fabs(period) > TConsts::epsilon) {
    Handler *handler = new Handler();
    track.handler    = handler;
    handler->track   = new TTrack(track);
    new Interpolator(*handler->track, period, amplitude, keepFirstPoint,
                     keepLastPoint);
  }
...
void TModifierJitter::modifyTracks(const TTrackList &tracks,
  ...
  for (int i = 0; i < cnt; ++i)
    if (i0 <= i && i < i1)
      modifyTrack(*tracks[i], outTracks);
    else
      TInputModifier::modifyTrack(*tracks[i], outTracks);
}
