...
TTrackPoint TModifierAssistants::Interpolator::interpolate(double index) {
  TTrackPoint p = track.original ? track.calcPointFromOriginal(index)
                                 : track.interpolateLinear(index);
  return guidelines.empty()
             ? p
             : guidelines.front()->smoothTransformPoint(p, magnetism);
}
  ...
  if (TInputManager *manager = getManager())
    if (TInputHandler *handler = manager->getHandler())
      return TAssistant::scanAssistants(
          handler->inputGetTool(), positions, positionsCount, outGuidelines,
          draw, enabledOnly, magnetism > 0, drawGuidelines, nullptr);
  return false;
}
...
void TModifierAssistants::modifyTrack(const TTrack &track,
        ...
        if (TTool *tool = handler->inputGetTool())
          if (TToolViewer *viewer = tool->getViewer()) {
            ...
            if (tool->getToolType() & TTool::LevelTool)
              if (TObjectHandle *objHandle =
                      TTool::getApplication()->getCurrentObject())
                if (!objHandle->isSpline())
                  trackToScreen *=
                      TScale(viewer->getDpiScale().x, viewer->getDpiScale().y);
            trackToScreen *= viewer->get3dViewMatrix().get2d();
            TGuidelineP guideline = TGuideline::findBest(
                intr->guidelines, track, trackToScreen, fixed);
  ...
  // fix points
  if (fixed || track.fixedFinished()) subTrack.fix_to(track.fixedSize());
...
void TModifierAssistants::draw(const TTrackList &tracks,
  ...
  scanAssistants(allHovers.empty() ? NULL : &allHovers.front(),
                 (int)allHovers.size(), nullptr, true, false, true);
