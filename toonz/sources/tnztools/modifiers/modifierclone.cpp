...
TTrackPoint TModifierClone::Interpolator::interpolateFromOriginal(
...
void TModifierClone::modifyTrack(const TTrack &track, TTrackList &outTracks) {
  if (!track.handler) {
    ...
    for (TTrackTransformList::const_iterator i = transforms.begin();
         i != transforms.end(); ++i) {
  ...
  if (handler->original) outTracks.push_back(handler->original);
  ...
  // process sub-tracks
  for (TTrackList::iterator i = handler->tracks.begin();
       i != handler->tracks.end(); ++i) {
    TTrack &subTrack = **i;
    Interpolator *intr =
...
void TModifierClone::modifyTracks(const TTrackList &tracks,
