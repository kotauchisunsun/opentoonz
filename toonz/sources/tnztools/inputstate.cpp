...
void printKey<TKey>(const TKey &k, std::ostream &stream) {
  stream << QKeySequence(k.key).toString().toStdString() << "[" << std::hex
         << k.key << std::dec;
...
public:
  ...
  static void print(const History &history, std::ostream &stream,
                    const std::string &tab) {
    const StateMap &states = history.getStates();
    stream << tab << "states: " << std::endl;
    for (typename StateMap::const_iterator i = states.begin();
         i != states.end(); ++i) {
    ...
    stream << tab << "locks: ";
    for (typename LockSet::const_iterator i = locks.begin(); i != locks.end();
         ++i) {
