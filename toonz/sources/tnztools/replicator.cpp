...
void TReplicator::updateTranslation() const {
...
int TReplicator::getMultipler() const { return 1; }

//---------------------------------------------------------------------------------------------------

void TReplicator::getModifiers(const TAffine &, TInputModifier::List &) const {}

//---------------------------------------------------------------------------------------------------

void TReplicator::getPoints(const TAffine &, PointList &) const {}

//---------------------------------------------------------------------------------------------------

TIntProperty *TReplicator::createCountProperty(const TStringId &id, int def,
                                               int min, int max) {
  if (min < 0) min = 0;
  if (def < min) def = min;
...
void TReplicator::transformPoints(const TAffine &aff, PointList &points, int i0,
                                  int i1) {
  ...
  for (int i = i0; i < i1; ++i) points.push_back(aff * points[i]);
}

//---------------------------------------------------------------------------------------------------

void TReplicator::drawReplicatorPoints(const TPointD *points, int count) {
...
int TReplicator::scanReplicators(TTool *tool, PointList *inOutPoints,
                                 TInputModifier::List *outModifiers, bool draw,
                                 bool enabledOnly, bool markEnabled,
                                 bool drawPoints, TImage *skipImage) {
                                ...
                                if (image->getType() == TImage::META)
                                  if (TMetaImage *metaImage =
                                    ...
                                    if (draw) {
                                      if (props)
                                        TAssistantBase::colorBase =
                                            toPixelD(props->getColorFilterColor(
                                                column->getColorFilterId()));
                                      TAssistantBase::colorBase.m *=
                                          column->getOpacity() / 255.0;
                                      glPushMatrix();
                                    ...
                                    for (TMetaObjectListCW::iterator i =
                                         ...
                                         i != reader->end(); ++i) {
                                      if (*i)
                                        if (const TReplicator *replicator =
                                                (*i)->getHandler<TReplicator>())
                                          if (!enabledOnly ||
                                              replicator->getEnabled()) {
                                            ...
                                            if (enabled) {
                                              ...
                                              if (multiplier * m >
                                                ...
                                                multiplier *= m;
                                                if (inOutPoints)
                                                  replicator->getPoints(
                                                      imageToTrack,
                                                      *inOutPoints);
                                                if (outModifiers)
                                                  replicator->getModifiers(
                                                      imageToTrack,
                                                      *outModifiers);
                                              }
                                            }
                                            ...
                                            if (draw) {
                                              unsigned int oldFlags =
                                                  TReplicator::drawFlags;
                                              if (enabled && outOfLimit)
                                                TReplicator::drawFlags |=
                                                    TReplicator::DRAW_ERROR;
                                              replicator->draw(
                                                  viewer,
                                                  enabled && markEnabled);
  ...
  if (drawPoints && inOutPoints && (int)inOutPoints->size() > inputPoints)
    drawReplicatorPoints(inOutPoints->data() + inputPoints,
