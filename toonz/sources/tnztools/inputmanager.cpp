...
void TInputModifier::modifyTrack(const TTrack &track, TTrackList &outTracks) {
  if (!track.handler) {
  ...
  TSubTrackHandler *handler =
      dynamic_cast<TSubTrackHandler *>(track.handler.getPointer());
  if (!handler) return;
...
bool TInputHandler::inputKeyEvent(bool press, TInputState::Key,
                                  QKeyEvent *event, const TInputManager &) {
  return press && event && inputKeyDown(event);
}

void TInputHandler::inputButtonEvent(bool press, TInputState::DeviceId,
...
void TInputHandler::inputHoverEvent(const TInputManager &manager) {
  if (!manager.getOutputHovers().empty())
    inputMouseMove(manager.getOutputHovers().front(), manager.state);
}

void TInputHandler::inputPaintTrackPoint(const TTrackPoint &point,
...
void TInputHandler::inputPaintTracks(const TTrackList &tracks) {
  ...
  while (true) {
    ...
    for (TTrackList::const_iterator i = tracks.begin(); i != tracks.end();
      ...
      if (t.pointsAdded > 0) {
        ...
        if (!track || (ticks - minTicks) * TToolTimer::frequency + timeOffset -
                              minTimeOffset <
                          0.0) {
          track         = *i;
          minTicks      = ticks;
    ...
    if (!track) break;  // all tracks are painted

    if (track->pointsAdded <= track->previewSize()) preview = true;
    if (!paintStarted) {
      inputPaintTracksBegin();
      paintStarted = true;
    }
  ...
  // end paint
  if (paintStarted) inputPaintTracksEnd();
}
...
void TInputManager::paintTracks() {
  ...
  // paint tracks
  if (changed && m_handler) m_handler->inputPaintTracks(subTracks);
...
const TTrackP &TInputManager::getTrack(TInputState::DeviceId deviceId,
  ...
  if (cmp < 0)
    return createTrack(a, deviceId, touchId, ticks, hasPressure, hasTilt);

  int b = (int)origTracks.size() - 1;
  cmp   = trackCompare(*origTracks[b], deviceId, touchId);
  if (cmp == 0) return origTracks[b];
  if (cmp > 0)
    return createTrack(b + 1, deviceId, touchId, ticks, hasPressure, hasTilt);
...
void TInputManager::addTrackPoint(const TTrackP &track, const TPointD &position,
                                  ...
                                  double time, bool final) {
  track->push_back(
      TTrackPoint(position, pressure, tilt, (double)track->size(), time,
...
void TInputManager::touchTracks(bool finish) {
  for (TTrackList::const_iterator i = m_tracks.front().begin();
    ...
    if (!(*i)->finished() && (*i)->size() > 0) {
      ...
      addTrackPoint(*i, p.position, p.pressure, p.tilt,
                    fixTicks(m_lastTicks) * TToolTimer::step, finish);
    }
  }
}
...
void TInputManager::setHandler(TInputHandler *handler) {
...
void TInputManager::trackEvent(TInputState::DeviceId deviceId,
                               TInputState::TouchId touchId,
                               const TPointD &position, const double pressure,
                               const TPointD &tilt, bool hasPressure,
                               bool hasTilt, bool final, TTimerTicks ticks) {
...
void TInputManager::hoverEvent(const THoverList &hovers) {
  if (&m_hovers[0] != &hovers) m_hovers[0] = hovers;
  for (int i = 0; i < (int)m_modifiers.size(); ++i) {
    m_hovers[i + 1].clear();
...
void TInputManager::draw() {
  ...
  if (debugInputManager) {
    ...
    for (int li = 0; li < cnt; ++li) {
      HSV2RGB(240 + li * 120.0 / (cnt - 1), 1, 1, &color[0], &color[1],
              &color[2]);
      glColor4dv(color);
      for (TTrackList::const_iterator ti = m_tracks[li].begin();
           ti != m_tracks[li].end(); ++ti) {
        assert(*ti);
        ...
        for (int i = 0; i < track.size(); ++i) {
          ...
          if (radius && li == 0) tglDrawCircle(*b, radius * pixelSize * 3);
        }
      }
    }
  }
