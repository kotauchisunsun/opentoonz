...
typedef void ReadPixelFunc(const void *pixel, float &colorR, float &colorG,
                           float &colorB, float &colorA);
typedef void WritePixelFunc(void *pixel, float colorR, float colorG,
                            float colorB, float colorA);
typedef bool AskAccessFunc(void *surfaceController, const void *surfacePointer,
...
inline bool dummyAskAccess(void *, const void *, int, int, int, int) {
  return true;
}
...
public:
  ...
  SurfaceCustom(void *pointer, int width, int height, int pixelSize,
                int rowSize = 0, void *controller = 0, bool antialiasing = true)
      : pointer(pointer)
      , width(width)
      , height(height)
      , pixelSize(pixelSize)
...
private:
  ...
  bool drawDabCustom(const Dab &dab, float *colorSummary) {
    ...
    int x1 =
        std::min(width - 1, (int)ceil(dab.x + dab.radius + 1.f - precision));
    int y0 = std::max(0, (int)floor(dab.y - dab.radius - 1.f + precision));
    int y1 =
        std::min(height - 1, (int)ceil(dab.y + dab.radius + 1.f - precision));
    ...
    // process
    for (int iy = h; iy;
         --iy, ddx += ddxNextRow, ddy += ddyNextRow, pixel += pixelNextRow)
      for (int ix = w; ix;
           --ix, ddx += ddxNextCol, ddy += ddyNextCol, pixel += pixelNextCol) {
        float o;
        if (enableAntialiasing) {
          float dd, dr;
          if (enableAspect) {
          ...
          float o0, o1;
          if (enableHardnessOne) {
            o0 = dd0 < -1.f ? -0.5f : 0.5f * dd0;
            o1 = dd1 < 1.f ? 0.5f * dd1 : 0.5f;
          } else if (enableHardnessHalf) {
            o0 = dd0 < -1.f  ? -0.25f
                 : dd0 < 0.f ? (0.25f * dd0 + 0.5f) * dd0
                             : (-0.25f * dd0 + 0.5f) * dd0;
            o1 = dd1 < 1.f ? (-0.25f * dd1 + 0.5f) * dd1 : 0.25f;
          } else {
            o0 = dd0 < -1.f        ? -kc2
        ...
        if (enableBlendColorize) {
          float dLum =
              destR * lr + destG * lg + destB * lb - blendColorizeSrcLum;
          float r = colorR + dLum;
          float g = colorG + dLum;
          float b = colorB + dLum;
    ...
    if (enableSummary) {
      ...
      colorSummary[3] =
          (float)(colorSumW > precision ? colorSumA / colorSumW : 0.0);
    }

    return true;
  }

  template <bool enableAspect, bool enableAntialiasing, bool enableHardnessOne,
            bool enableHardnessHalf, bool enableBlendNormal,
            bool enableBlendLockAlpha>
  inline bool drawDabCheckBlendColorize(const Dab &dab) {
    if (dab.colorize > precision) {
      return drawDabCustom<enableAspect, enableAntialiasing, enableHardnessOne,
                           enableHardnessHalf,
                           false,  // enablePremult
                           enableBlendNormal, enableBlendLockAlpha,
  ...
  inline bool drawDabCheckBlendLockAlpha(const Dab &dab) {
    if (dab.lockAlpha > precision) {
      return drawDabCheckBlendColorize<enableAspect, enableAntialiasing,
                                       enableHardnessOne, enableHardnessHalf,
                                       enableBlendNormal,
  ...
  inline bool drawDabCheckBlendNormal(const Dab &dab) {
    if ((1.f - dab.lockAlpha) * (1.f - dab.colorize) > precision) {
      return drawDabCheckBlendLockAlpha<enableAspect, enableAntialiasing,
                                        enableHardnessOne, enableHardnessHalf,
  ...
  template <bool enableAspect, bool enableAntialiasing>
  inline bool drawDabCheckHardness(const Dab &dab) {
    if (dab.hardness >= 1.f - precision) {
      return drawDabCheckBlendNormal<enableAspect, enableAntialiasing,
                                     true,  // enableHardnessOne
                                     false  // enableHardnessHalf
                                     >(dab);
    } else if (fabsf(dab.hardness - 0.5f) <= precision) {
      return drawDabCheckBlendNormal<enableAspect, enableAntialiasing,
  ...
  inline bool drawDabCheckAntialiasing(const Dab &dab, bool antialiasing) {
    if (antialiasing) {
      return drawDabCheckHardness<enableAspect,
  ...
  inline bool drawDabCheckAspect(const Dab &dab, bool antialiasing) {
    if (dab.aspectRatio > 1.f + precision) {
      return drawDabCheckAntialiasing<true  // enableAspect
                                      >(dab, antialiasing);
    } else {
      return drawDabCheckAntialiasing<false  // enableAspect
                                      >(dab, antialiasing);
    }
  }
...
public:
  bool getColor(float x, float y, float radius, float &colorR, float &colorG,
                float &colorB, float &colorA) {
    float color[4];
    bool done = drawDabCustom<false,  // enableAspect
                              false,  // enableAntialiasing
                              false,  // enableHardnessOne
                              true,   // enableHardnessHalf
                              false,  // enablePremult
  ...
  bool drawDab(const Dab &dab) {
    ...
    if (d.radius <= precision) return true;
    if (d.hardness <= precision) return true;
    ...
    // check opaque
    if (d.opaque < minOpaque) return false;
