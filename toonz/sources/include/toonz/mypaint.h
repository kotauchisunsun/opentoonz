...
public:
  ...
  Dab(float x, float y, float radius, float colorR = 0.f, float colorG = 0.f,
      float colorB = 0.f, float opaque = 1.f, float hardness = 0.5f,
      float alphaEraser = 1.f, float aspectRatio = 1.f, float angle = 0.f,
...
class Surface {
  ...
  static int internalDrawDab(MyPaintSurface *self, float x, float y,
                             float radius, float colorR, float colorG,
                             float colorB, float opaque, float hardness,
                             float alphaEraser, float aspectRatio, float angle,
                             float lockAlpha, float colorize) {
    return static_cast<InternalSurface *>(self)->m_owner->drawDab(
        Dab(x, y, radius, colorR, colorG, colorB, opaque, hardness, alphaEraser,
            aspectRatio, angle, lockAlpha, colorize));
  }

  static void internalGetColor(MyPaintSurface *self, float x, float y,
                               float radius, float *colorR, float *colorG,
                               float *colorB, float *colorA) {
    static_cast<InternalSurface *>(self)->m_owner->getColor(
        x, y, radius, *colorR, *colorG, *colorB, *colorA);
  }
...
public:
  ...
  virtual bool getColor(float x, float y, float radius, float &colorR,
                        float &colorG, float &colorB, float &colorA) = 0;
...
public:
  Brush() : m_brush(mypaint_brush_new()) { fromDefaults(); }

  Brush(const Brush &other) : m_brush(mypaint_brush_new()) { fromBrush(other); }

  ~Brush() { mypaint_brush_unref(m_brush); }
  ...
  void reset() { mypaint_brush_reset(m_brush); }

  void newStroke() { mypaint_brush_new_stroke(m_brush); }

  int strokeTo(Surface &surface, float x, float y, float pressure, float xtilt,
               float ytilt, double dtime) {
    return mypaint_brush_stroke_to(m_brush, &surface.m_surface, x, y, pressure,
                                   xtilt, ytilt, dtime);
  }
  ...
  void getMappingPoint(MyPaintBrushSetting id, MyPaintBrushInput input,
  ...
  void fromDefaults() { mypaint_brush_from_defaults(m_brush); }
...
public:
    ...
    if (!initialized) {
      for (int i = 0; i < MYPAINT_BRUSH_SETTINGS_COUNT; ++i) {
        const MyPaintBrushSettingInfo *info =
            mypaint_brush_setting_info((MyPaintBrushSetting)i);
  ...
  static const Setting &byId(MyPaintBrushSetting id) { return all()[id]; }
...
private:
  ...
  Input(const Input &)
      : id(), hardMin(), softMin(), normal(), softMax(), hardMax() {}
...
public:
    ...
    if (!initialized) {
      for (int i = 0; i < MYPAINT_BRUSH_INPUTS_COUNT; ++i) {
        const MyPaintBrushInputInfo *info =
            mypaint_brush_input_info((MyPaintBrushInput)i);
  ...
  static const Input &byId(MyPaintBrushInput id) { return all()[id]; }

  static const Input *findByKey(const std::string &key) {
    const Input *inputs = all();
    for (int i = 0; i < MYPAINT_BRUSH_INPUTS_COUNT; ++i)
      if (inputs[i].key == key) return &inputs[i];
    return 0;
  }
};
}  // namespace mypaint
