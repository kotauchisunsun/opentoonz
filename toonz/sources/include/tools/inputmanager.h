...
public:
  ...
  virtual void modifyTrack(const TTrack &track, TTrackList &outTracks);
  virtual void modifyTracks(const TTrackList &tracks, TTrackList &outTracks);
...
public:
  ...
  virtual bool inputKeyEvent(bool press, TInputState::Key key, QKeyEvent *event,
                             const TInputManager &manager);

  virtual void inputButtonEvent(bool press, TInputState::DeviceId device,
  ...
  virtual void inputPaintTrackPoint(const TTrackPoint &point,
...
private:
  ...
  void addTrackPoint(const TTrackP &track, const TPointD &position,
                     double pressure, const TPointD &tilt, double time,
                     bool final);
...
public:
  ...
  TInputHandler *getHandler() const { return m_handler; }
  ...
  void addModifiers(const TInputModifier::List &modifiers) {
    for (TInputModifier::List::const_iterator i = modifiers.begin();
         i != modifiers.end(); ++i)
      addModifier(*i);
  }
  ...
  void trackEvent(TInputState::DeviceId deviceId, TInputState::TouchId touchId,
                  ...
                  const TPointD &tilt, bool hasPressure, bool hasTilt,
