...
private:
  ...
  Pointer makeChainWithout(const Pointer &state) {
    return state == this ? previous
           : previous    ? Pointer(new TKeyStateT(
                               previous->makeChainWithout(state), ticks, value))
                         : this;
  }
...
public:
  ...
  Pointer change(bool press, const Type &value, TTimerTicks ticks) {
    ...
    if (press == bool(p)) return Pointer(this);
...
private:
  ...
  void autoRemove() {
    ...
    if (!m_locks.empty()) ticks = std::min(ticks, *m_locks.begin());
    while (true) {
...
public:
  ...
  inline const StateMap &getStates() const { return m_states; }
  inline const LockSet &getLocks() const { return m_locks; }
};
